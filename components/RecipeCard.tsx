...
import { BookMarkedIcon, PieChartIcon, CheckIcon } from './Icons';
...
const RecipeCard: React.FC<RecipeCardProps> = ({ recipe }) => {
  const { name, description, ingredients, yield: recipeYield, dailyValues } = recipe;

  // Generate a random ID for the image to avoid caching
  ...
  return (
    <div className="bg-gray-800 rounded-xl overflow-hidden shadow-lg hover:shadow-indigo-500/30 transform hover:-translate-y-1 transition-all duration-300 flex flex-col">
      <img 
        src={`https://picsum.photos/seed/${name.replace(/\s/g, '')}${randomImageId}/600/400`} 
        alt={name} 
